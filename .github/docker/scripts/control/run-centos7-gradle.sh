...
docker exec -i $DOCKER_CONTAINER_CENTOS7_BUILDER_GRADLE_NAME find $BUILDER_WORKING_REPO_DIR/.github/docker/scripts -type f -iname "*.sh" -exec chmod +x {} \; 

# Wait for builder to be ready
until [ "`docker exec -i $DOCKER_CONTAINER_CENTOS7_BUILDER_GRADLE_NAME ls -alR $BUILDER_WORKING_REPO_DIR/.github/docker/scripts`" ]; do
    sleep 5;
done;
